...
from utility.utilities import (
    rel_link_last_file,
    # get_file_new_link_and_timestamp,
    get_link_and_timestamp,
)
...
def train(args, dyn_stop=False):
    ...
    prev_round_avg_loss = sys.maxsize
    for idx_round in range(args.n_rounds):
        ...
        triples_link, triples_time = get_link_and_timestamp(args.triples)
        ...
        for batch_idx, BatchSteps in zip(range(start_batch_idx, maxsteps), reader):
            ...
            if args.rank < 1:
                ...
                manage_checkpoints(args,
                                   ...
                                   num_per_epoch,
                                   epoch_idx,
                                   prefix=f".round_{idx_round}")

        rel_link_last_file(path, "colbert-LAST.dnn", "*.model")
        Run.info(f"Avg loss: {avg_loss}")
        elapsed = float(time.time() - start_time)
